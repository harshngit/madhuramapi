...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/lodha-invoice/:id/history — who created/updated/deleted this invoice, and when
// ─────────────────────────────────────────────────────────────────────────────
/**
 * @swagger
 * /api/lodha-invoice/{id}/history:
 *   get:
 *     summary: Get the create/update/delete history for a Lodha invoice (who did what, and when)
 *     tags: [Lodha Invoice]
 *     parameters:
 *       - in: path
 *         name: id
...
router.get("/:id/history", async (req, res) => {
  try {
    const data = await getEntityHistory("lodha_invoice", req.params.id, {
      limit: req.query.limit, offset: req.query.offset,
    });
    res.json(data);
  } catch (err) {
    console.error("Error fetching Lodha invoice history:", err);
